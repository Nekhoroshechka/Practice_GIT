...
def read_pyproject_toml(
    ...
    if not value:
        value = find_pyproject_toml(
            ctx.params.get("src_", ()), ctx.params.get("stdin_filename", None)
        )
        if value is None:
            return None
...
def path_empty(
    ...
    """
    Exit if there is no `src_` provided for formatting
    """
    if not src:
        if verbose or not quiet:
            out(msg)
...
def reformat_one(
    ...
    *,
    ...
    """Reformat a single file under `src_` without spawning child processes.
...
def format_file_in_place(
    ...
    *,
    ...
    """Format file under `src_` path. Return True if changed.
...
def assert_equivalent(src: str, dst: str) -> None:
    """Raise AssertionError if `src_` and `dst` aren't equivalent."""
    try:
    ...
    if src_ast_str != dst_ast_str:
        log = dump_to_file(diff(src_ast_str, dst_ast_str, "src_", "dst"))
        raise ASTSafetyError(
