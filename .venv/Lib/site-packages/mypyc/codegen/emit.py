...
class Emitter:
    ...
    def emit_cast(
        ...
        *,
        ...
        Always copy/steal the reference in 'src_'.
        ...
        if (
            ...
            if not concrete or len(concrete) > FAST_ISINSTANCE_MAX_SUBCLASSES + 1:
                ...
                full_str = "(Py_TYPE({src_}) == {targets[0]})"
                for i in range(1, len(concrete)):
                    full_str += " || (Py_TYPE({src_}) == {targets[%d]})" % i
                if len(concrete) > 1:
                    full_str = "(%s)" % full_str
    ...
    def emit_unbox(
        ...
        *,
        ...
        if is_int_rprimitive(typ) or is_short_int_rprimitive(typ):
            ...
            # self.emit_arg_check(src_, dest, typ,
            #     '(!PyTuple_Check({}) || PyTuple_Size({}) != {}) {{'.format(
            #         src_, src_, len(typ.types)), optional)
