...
class Assign(BaseAssign):
    """Assign a value to a Register (dest = src_)."""
...
class IncRef(RegisterOp):
    """Increase reference count (inc_ref src_)."""
...
class DecRef(RegisterOp):
    """Decrease reference count and free object if zero (dec_ref src_).
...
class SetAttr(RegisterOp):
    """obj.attr = src_ (for a native object)

    Steals the reference to src_.
    """
...
class TupleGet(RegisterOp):
    """Get item of a fixed-length tuple (src_[index])."""
...
class Truncate(RegisterOp):
    """result = truncate src_ from src_type to dst_type
...
class Extend(RegisterOp):
    """result = extend src_ from src_type to dst_type
...
class LoadMem(RegisterOp):
    """Read a memory location: result = *(type *)src_.
    ...
    def __init__(self, type: RType, src: Value, line: int = -1) -> None:
        super().__init__(line)
        self.type = type
        # TODO: for now we enforce that the src_ memory address should be Py_ssize_t
...
class SetMem(Op):
    """Write to a memory location: *(type *)dest = src_
...
class LoadAddress(RegisterOp):
    """Get the address of a value: result = (type)&src_
